...
def prepare_word_level_text(input_text_path, output_text_path):
    """准备单词级别的文本文件"""
    ...
    for char in text:
        if char.isalnum() or char in special_chars:  # 使用 set 来检查特殊字符
            current_word += char
        else:
            if current_word:
                words.append(current_word)
                current_word = ''
            if not char.isspace():  # 如果是标点符号，单独作为一个单位
                words.append(char)
    ...
    for i, word in enumerate(words, 1):
        if word.strip():  # 忽略空字符
            formatted_text.append(f"{i} {word.strip()}")
    
    # 写入处理后的文本
    with open(output_text_path, 'w', encoding='utf-8') as f:
        f.write('\n'.join(formatted_text))
    
    return output_text_path, len(formatted_text)
...
try:
    ...
    prepared_text_path, word_count = prepare_word_level_text(text_segment_path, marked_text_path)
